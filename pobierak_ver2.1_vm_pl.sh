...
printMenu(){
	...
	usb_check
	echo ""
    echo -e "${normal} ${u}Pobierak_ver2.1${NC}"
	echo ""
	...
	echo ""
    echo -e "${BLUE}	\t6)	SCIAGNIJ FILM & SCIEZKE MP3 ${NC}"
    echo ""
	echo -e "${CYAN} 	\t7)	INSTALACJA YOUTUBE_DL ${NC}"
	echo ""
	echo -e "${CYAN}	\t8)	UPGRADE BIBLIOTEK DO KONWERTOWANIA ${NC}"
	echo ""
    echo -e "${CYAN}        \t9)      INSTALUJ POBIERAKA W KONSOLI  ${NC}"
    echo ""
    echo -e "${CYAN} 	\t10)	YOUTUBE-DL UPDATE ${NC}"
    echo ""
...
runOption(){
	case $option in
		...
		4) download_channel;;
		5) download_movie;;
        6) download_movie_and_music_from_file;;
        7) install_ytdl;;
		8) upgrade_ytdl;;
		9) wpis_bash;;
		10) youtube_dl_update;;
        11) install_tor;;
		12) exit;;
